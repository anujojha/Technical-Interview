...
class HtWt implements Comparable {
	...
	public int compareTo(Object s) {
        ...
        // if the height is same, check the weight 
        else {		
...
public class mySort {
    ...
    public static int partition1(int left, int right , int[] arr) {
	...
	static int[] bucketSort(int[] a, int maxVal){
	...
	private static void longestIncreasingSubsequence( ArrayList<HtWt> array, ArrayList<HtWt>[] solutions, int currentindex ) {
		...
		HtWt currentelement = array.get( currentindex );
		...
		for (int i = 0; i < currentindex; i++) {
			...
			if ( array.get(i).isBefore(currentelement) ) { // If currentelement is bigger than list tail

				bestsequence = seqWithMaxLength(bestsequence, solutions[i]); // Set bestsequence to our new max
			}
		}
